    ...
    UNUserNotificationCenterDelegate
{
    ...
    @Published var channels: [Channel] {
        didSet { PersistenceService.saveChannels(channels) }
    }
    ...
    @Published var channelConversations: [UInt8: [Message]] {
        didSet {
            PersistenceService.saveChannelConversations(channelConversations)
        }
    }
    ...
    private var ackCodeToContext:
        [UInt32: (messageID: UUID, contactKey: Data)] = [:]
    ...
    override init() {
        ...
        self.channelConversations =
            PersistenceService.loadChannelConversations()
    ...
    @objc private func onBleReady() {
        Logger.shared.log(
            "MessageService: Received BLE Ready signal. Initializing session..."
        )
        sendAppStart()
    ...
    private func handleProtocolPacket(_ data: Data) {
        ...
        case 6:  // RESP_CODE_SENT
            let ackCode = data.subdata(in: 2..<6).to(type: UInt32.self)
                .littleEndian
            if let context = ackCodeToContext[ackCode] {
                Logger.shared.log(
                    "Message \(context.messageID) marked as 'Sent'."
                )
                updateMessageStatus(
                    for: context.contactKey,
        ...
        case 0x82:  // PUSH_CODE_SEND_CONFIRMED
            ...
            if let context = ackCodeToContext[ackCode] {
                Logger.shared.log(
                    "Message \(context.messageID) marked as 'Delivered'."
                )
                updateMessageStatus(
                    for: context.contactKey,
                    messageID: context.messageID,
        ...
        case 5:  // RESP_CODE_SELF_INFO
            ...
            newSettings.radioFreq =
                data.subdata(in: 48..<52).to(type: UInt32.self).littleEndian
            newSettings.radioBw =
                data.subdata(in: 52..<56).to(type: UInt32.self).littleEndian
            newSettings.radioSf = data[56]
            newSettings.radioCr = data[57]
            newSettings.name =
                String(
                    data: data.subdata(in: 58..<data.count),
                    encoding: .utf8
            ...
            DispatchQueue.main.async {
                ...
                Logger.shared.log(
                    "   -> Node settings updated from device. Name: \(newSettings.name)"
                )
            }
            break
        case 12:  // RESP_CODE_BATT_AND_STORAGE
            ...
            let milliVolts = data.subdata(in: 1..<3).to(type: UInt16.self)
                .littleEndian
            let usedStorage = data.subdata(in: 3..<7).to(type: UInt32.self)
                .littleEndian
            let totalStorage = data.subdata(in: 7..<11).to(type: UInt32.self)
                .littleEndian

            Logger.shared.log(
                "Received battery level and storage: \(milliVolts) mV, \(usedStorage)KB/\(totalStorage)KB"
            )
            DispatchQueue.main.async {
        ...
        case 0x83:
            Logger.shared.log(
                "Notification received: a message is waiting. Requesting it now."
            )
            syncNextMessage()
        default:
    ...
    func sendMessage(to contact: Contact, message: String) {
        ...
        DispatchQueue.main.async {
            self.conversations[contact.publicKey, default: []].append(
                newMessage
            )
        }
        BLEManager.shared.writeData(frame)
    ...
    func sendChannelMessage(to channel: Channel, message: String) {
        ...
        DispatchQueue.main.async {
            self.channelConversations[channel.id, default: []].append(
                newMessage
            )
        }

        Logger.shared.log(
            "Sending CMD_SEND_CHANNEL_TXT_MSG to #\(channel.name)..."
        )
        var frame = Data()
        frame.append(3)
        frame.append(0)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            if let index = self.channelConversations[channel.id]?.firstIndex(
                where: {
                    $0.id == newMessage.id
                })
            {
    ...
    private func parseContact(from data: Data) {
        ...
        let name =
            String(bytes: nameData, encoding: .utf8)?.replacingOccurrences(
                of: "\0",
                with: ""
            )
        ...
        if let existingContactIndex = pendingContacts.firstIndex(where: {
            ...
            Logger.shared.log(
                "Added new contact: \(receivedContact.name) (\(receivedContact.publicKey.hexEncodedString()))"
            )
        }
    }
    ...
    private func parseReceivedMessage(from data: Data) {
        ...
        let messageText =
            String(
                data: data.subdata(in: textOffset..<data.count),
                encoding: .utf8
            )
            ?? "Unreadable message"

        if let contact = contacts.first(where: {
            $0.publicKey.prefix(6) == pubkeyPrefix
        }) {
            let newMessage = Message(
                content: messageText,
            ...
            Logger.shared.log(
                "Successfully parsed message '\(messageText)' from \(contact.name)"
            )
            ...
            showNewMessageNotification(
                ...
                body: messageText,
                userInfo: [
                    "contactPublicKeyHex": contact.publicKey.hexEncodedString()
                ]
            )
            ...
            DispatchQueue.main.async {
                ...
                let currentCount =
                    self.unreadMessageCounts[contact.publicKey] ?? 0
    ...
    private func parseChannelMessage(from data: Data) {
        ...
        guard let channel = self.channels.first(where: { $0.id == channelID })
            ...
            Logger.shared.log(
                "Warning: Received message for unknown channel ID \(channelID)"
            )
            return
        }
    ...
    func markConversationAsRead(for contactKey: Data) {
        ...
        if wasModified {
            ...
            Logger.shared.log(
                "Conversation with \(contactKey.hexEncodedString()) marked as read."
            )
        }
    }
    ...
    func hasUnreadMessages(in channelID: UInt8) -> Bool {
        return channelConversations[channelID]?.contains(where: {
    ...
    func userNotificationCenter(
        ...
        if let publicKeyHex = userInfo["contactPublicKeyHex"] as? String {
            Logger.shared.log(
                "Notification tapped for contact key: \(publicKeyHex)"
            )
            ...
            Logger.shared.log(
                "Notification tapped for channel ID: \(channelID)"
            )
            if let targetChannel = self.channels.first(where: {
                $0.id == channelID
            }) {
                DispatchQueue.main.async {
